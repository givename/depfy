...
export type InjectedDescriptor<T = any, O = any> = {
  name: string;
  token: string;
  clone(props: { options?: O; name?: string }): InjectedDescriptor<T, O>;
  [DI_OPTIONS]: O;
  [DI_INJECTED]: T /** metafield */;
};

export type InferInjectedDependency<T extends InjectedDescriptor> =
  T extends InjectedDescriptor<infer T> ? T : never;

export type InferInjectedDependencies<
  T extends Record<string, InjectedDescriptor>
> = {
